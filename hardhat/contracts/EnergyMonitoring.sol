...
contract EnergyMonitoring {
    ...
    enum EnergyType { Solar, Wind, Biomass, Tidal }
    ...
    event EnergyProduced(address indexed producer, uint256 amount, EnergyType energyType, uint256 timestamp);
     ...
     */
    function recordProduction(address producer, uint256 amount, EnergyType energyType) external onlyOwner {
        require(amount > 0, "Produced amount must be greater than zero");

        totalProduced[producer] += amount;
        energyRecords[producer].push(EnergyRecord({
            produced: amount,
            consumed: 0,
     ...
     */
    function recordConsumption(address consumer, uint256 amount) external onlyOwner {
        require(amount > 0, "Consumed amount must be greater than zero");

        totalConsumed[consumer] += amount;
        energyRecords[consumer].push(EnergyRecord({
            produced: 0,
            consumed: amount,
            timestamp: block.timestamp,
     ...
     */
    function getRecords(address user) external view returns (EnergyRecord[] memory) {
